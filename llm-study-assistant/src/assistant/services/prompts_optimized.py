...
- 避免展示详细推理过程。
- final_answer 必须准确引用 contexts 中的信息，不要编造内容。
- supporting_sources 必须列出实际使用的文档片段ID。
"""
...
输出格式：
{"type":"single_choice","final_answer":"选项的完整文本...","confidence":0.95,"brief_rationale":"基于context中的具体信息...","supporting_sources":["doc#1", "..."]}

注意事项：
- 必须仔细比对每个选项与 contexts 中的信息
- 只有当 contexts 中明确支持某个选项时才选择它
- 如果没有足够的证据支持任何一个选项，将 confidence 设置为较低值并说明原因"""
...
输出格式：
{"type":"multi_choice","final_answer":["A","C"],"confidence":0.9,"brief_rationale":"选项A和C在context中有明确支持...","supporting_sources":["doc#1","..."]}

注意事项：
- 必须分别验证每个选项的正确性
- 只选择 contexts 中明确支持的选项
- 如果没有足够的证据支持任何选项，将 confidence 设置为较低值并说明原因"""
...
输出格式：
{"type":"true_false","final_answer":"True|False","confidence":0.9,"brief_rationale":"context中明确提到...","supporting_sources":["doc#1","..."]}

注意事项：
- 必须基于 contexts 中的具体信息进行判断
- 如果 contexts 中没有明确支持或反驳题目陈述，将 confidence 设置为较低值并说明原因"""
...
输出格式：
{"type":"subjective","final_answer":"基于context的准确信息回答...","confidence":0.85,"brief_rationale":"根据context中的关键信息...","supporting_sources":["doc#1","..."]}

注意事项：
- 答案必须严格基于 contexts 中的信息
- 不要添加 contexts 中没有提及的内容
- 如果 contexts 中没有足够信息，明确说明并降低 confidence"""
